...
excel_file_name             =   'input_output/heur_vs_gch_test_matrix_'+ str(number_of_groups) + '.xlsx'
    ...
    for iter in range(num_sols_to_investigate):
        ...
        for n in  nScens:
            ...
            for seed in seeds:     
                ...
                results_i_m         =   copy.deepcopy(results)
                input               =   generate_scenarios(input, n, seed) 
                input
                results_i_m         =   run_model_mip_fixed(input,results_i,30,print_optimizer = False,create_model_and_warmstart_file=True,MIPgap_limit=True,MIPgap_value=0.01)
                ...
                cell                =   alphabeth[col_index]+str(current_row_outlay + num_seeds_per_solution +2 + seed)
                sheet[cell]         =   results_i_m["obj"]
                
        col_index                   =   0
        current_row_outlay          +=  len(seeds)*3 + 5
